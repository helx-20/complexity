...
class ToyNet(nn.Module):
    ...
    def __init__(self, K=256):
        ...
        self.K = K
        self.neuron = LIF_neuron(256,512*4) 
        
        self.encode = nn.Sequential(
            #self.neuron,
            nn.Linear(1296, 2*self.K))
        
        for i in range(2):
            setattr(self, 'task_{}'.format(i), nn.Linear(512*4, 10))
    ...
    def forward(self, x, num_sample=1):
        ...
        encoding = torch.mean(self.neuron(self.reparametrize_n(mu,std,num_sample)),dim=-2)
