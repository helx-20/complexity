...
class lif_hh(nn.Module):
    ...
    def forward(self,input,mem,spike):
        ...
        hedden_size = self.out_features
        self.mul_op += 3*input_size*hedden_size 
        self.add_op += 3*(input_size-1)*hedden_size
...
class SNN_Model_LIF_hh(nn.Module):
    ...
    def forward(self, input, win=15):
        ...
        self.add_op += self.lif_4.add_op
        self.mul_op += self.lif_4.mul_op

        self.mul_op += 4*cfg_fc[0]*cfg_fc[1] 
        self.mul_op += self.win*15*cfg_fc[0]#inside neuron
        self.add_op += (4*cfg_fc[0]-1)*cfg_fc[1] 
        self.add_op += self.win*14*cfg_fc[0]#inside neuron
